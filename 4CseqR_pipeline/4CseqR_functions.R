...
fragment.ends <- function (filteredLibraryName, fragment__start_min,fragment_start_plus,fragment_end_plus,fragment_end_min) {
  #read the file
  MyData <- read.table (filteredLibraryName,header = TRUE)

  #combine the fragments ends 
  x <- cbind(fragment__start_min,fragment_start_plus,fragment_end_plus,fragment_end_min)
...
select.reads  <- function (filename,PrimerSeq,RestrEnzyme,pattern){
  ...
  match <- regexpr(pattern=RestrEnzyme[1], text=good.reads)
  if (match >= 0) {
