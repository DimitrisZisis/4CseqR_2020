...
newdata.dt <- fread("totalnorm_all_arab_qnorm_zero.txt")
...
LMMPerWin <- function(sliding_window, fragment_table, window_numb){
  ...
  #datav = data.table(fragnum, flabel, fsample, whichFragments, data1)
  # run kruskalwallis test
...
##############################################################################################################################
#test for LMM model 
newtab5 <- read.csv(file="sliding_LMM_test2.csv", header=T, sep=",")
head(newtab5)
library(lmerTest)
