...
LMM_data <- read.csv("LMM_emmeans_200000_50_varf_qnorm_arab.csv",header = T, sep="\t")
LMM_data[1:10,]
Fisher_data <- read.csv("Fisher_bintest_200000_50_all_fdr_arab.csv",header = T, sep="\t")
Fisher_data[1:10,]
...
# first way to see the means and probabilities 
#plot(Variant_A, Variant_B, col=ifelse(Variant_A > 0.1, 2, 1)) +
#  title(main="Mean fragment coverage")
LMM_data$DCW <- "NO"
