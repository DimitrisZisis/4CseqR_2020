...
newdata <- read.csv(file="/4CseqR-master/4CseqR/Library_Fragments/iPSC_pcdhb19_3_salmon.csv", header=T, sep="\t")
newdata[1:10,]
...
for (kk in 1:nlev) {
  ...
  print(U3)
  write.table(U3, file = "iPSC_pcdhb19_3_blind_fend_blind_new.txt", append=TRUE , quote = FALSE,  sep = "\t", row.names=FALSE , col.names=FALSE)
}

#Sort the table and produce total normalized counts 
newtab<- read.table(file= "iPSC_pcdhb19_3_blind_fend_blind_new.txt", header=FALSE, sep="\t")
newtab[1:10, ]
...
#totnormal[1:10]
write.table(chrsort, file = "ranksort_iPSC_pcdhb19_3_blind_fend_blind_new.txt" , quote = FALSE,  sep = " ", row.names=FALSE , col.names=FALSE)
